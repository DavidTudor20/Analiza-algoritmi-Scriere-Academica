...
inline void runBenchmarks(int choice) {
    ...
    outFile << "Distribution,Size,Runs,Bubble (us),Selection (us),Insertion (us),Merge (us)\n";
    ...
    for (const auto& dist : distributions) {
        ...
        for (int size : sizes) {
            int runs = (size <= 100) ? 10000 : 5; 
            vector<int> data = dist.second(size);

            double timeBubble = 0, timeSelect = 0, timeInsert = 0, timeMerge = 0;
